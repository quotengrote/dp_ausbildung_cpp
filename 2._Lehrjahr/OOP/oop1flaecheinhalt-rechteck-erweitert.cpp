...
public:  //Beginn Klassendeklaration
	...
	float get_laenge();
	float get_breite();
	string get_raumname(); //String weil Text agespeichert wird
	string get_belagart();
...
Raum::Raum(){
	laenge=0; //Definition der Anfangswerte, jedes neue Objekt wird mit diesen Werten erstellt
	breite=0;
	flaeche=0;
}
...
void Raum::set_laenge(float l){ 	//Übergibt die "Übergabewert(beim Aufruf der Methode) 
...
int main(){
	...
	//Aufruf der Methoden mit den lokalen Variablen an das Objekt: raum1, der Klasse Raum
	...
	//Rückgabewert aus den Klassenvariablen berechnet
	raum1.berechnung();
